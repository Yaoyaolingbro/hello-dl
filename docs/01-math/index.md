...
本章共 **30 个小节**，顺序经过仔细设计——每节只依赖前面已学的内容。
...
| 章节 | 小节数 | 核心内容 | 主要引用章节 |
|------|--------|----------|-------------|
| [线性代数](linear-algebra/index.md) | 9 | 矩阵运算 → 求导 → 分解 → 复数 | 全部 |
| [几何基础](geometry/index.md) | 3 | 刚体运动 → 空间变换 → 曲线设计 | 3DV、机器人、等变网络 |
| [概率论](probability/index.md) | 9 | 分布 → 推断 → 信息论 → 随机过程 → ELBO | 生成模型、RL |
| [优化理论](optimization/index.md) | 9 | 梯度 → 收敛 → 约束 → 插值 → ODE | 训练、机器人控制 |
