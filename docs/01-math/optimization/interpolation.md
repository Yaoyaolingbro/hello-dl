...
$k$ 是次数，$\mathbf{P}_i$ 是控制点，$B_{i,k}$ 由 de Boor 递推公式定义（每个基函数只在 $k+1$ 个节点区间上非零）。B 样条的**局部支撑性**使得移动一个控制点只影响局部曲线形状，而不影响整体，这是设计工具（CAD、字体）的核心需求。几何章节的贝塞尔曲线节会详细展开 B 样条与贝塞尔的关系。
