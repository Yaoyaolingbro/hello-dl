...
> 2026-08-16 更新：项目进入主线教材重写阶段。当前生效的架构决策见
> `docs/superpowers/specs/2026-08-16-mainline-rewrite-design.md`。后续编写优先以该文档为准；
> 本文更早的目录和分支说明只用于理解历史状态。

## 2026-08-16 重写决策摘要

- 保留 MkDocs Material 框架、四个 Part 和 Part 4 现有科研方向。
- 全书采用“主线教材 + 标签化面试/科研内容”。
- Part 1 定位为已有数学基础读者使用的 AI 数学工具箱。
- 传统机器学习从 Part 1 移到 Part 2；Part 2 覆盖入门、训练闭环和基础面试。
- Part 3 删除 imitation learning，聚焦表征学习、现代架构、生成模型、后训练和强化学习。
- Part 4 各方向的 `index.md` 兼任速览页，不新建重复的速览章节。
- 写作应自然、简洁；关键公式给出必要推导，但不堆砌公式。
- 每次写章节前先概览全书和相邻章节，使用交叉引用减少重复。
- 知乎和博客可用于寻找解释角度；技术事实需要教材、论文或官方资料交叉验证。

版本策略：重写前状态保存在 `archive/pre-rewrite-20260816`，实际重写在
`rewrite/part2-mainline` 分支进行。完成并验证后再合并到 `main`。
